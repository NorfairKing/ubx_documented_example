...
*** Generating the code for your package
    ...
    ~/ubx/microblx_cmake $ ./generate_pkg.lua -s ~ubx/ubx_example/models/example.pkg
    #+END_EXAMPLE
    
    You can add a '-d' option to this command, and specify a directoy, to override the 'path' option in the package model
